...
const input_types = <const> [ "date", "text", "number", "file", "image", "name"];
...
export interface anchor {
    ...
    element:HTMLElement,
    //
    //The page in which this io resides. This is important for naigating outside
    //of the page where he io is located. We need to be able to restore this page
    //when we come back. For instance, editing a foreign key from an exploer
...
export abstract class io extends outlook.view{
    ...
    public original_value?:library.basic_value;
    //
    constructor(
        //
        ...
        this.button = this.create_element(
           `input`, anchor.element, {
                ...
                onclick:async(evt)=>{
                    ...
                    //requested
                    if (this.use_explorer) await this.explore();
                    else {
                        //
...
export class file extends input{
    ...
    public async browse(
       ...
       initial_type:'file'|'folder'
        ...
        const content_out = <tree.directory.child>await explorer.administer();
        //
        //Only update the td if the selection was successful
        if (content_out===undefined) return;
        //
        //Extract the path from the directory content
        const path:string = content_out.path;
        //
